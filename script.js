document.addEventListener("DOMContentLoaded", function () {
  ...
  async function cargarEstaciones() {
    try {
      ...
      loadingIndicator.style.display = "none";
      playlist.innerHTML =
        '<p style="color: red;">Error al cargar la lista de estaciones.</p>';
    }
  }
  ...
  function getBotResponse(userInput) {
    ...
    const oldButtons = document.querySelectorAll(".option-btn:not(:disabled)");
  ...
  function loadChatHistory() {
    ...
    // Limpiar el mensaje de bienvenida inicial si hay historial
    if (history.length > 0) {
      chatBody.innerHTML = "";
      history.forEach((message) => {
        // Re-renderizar mensajes sin volver a pedir el nombre
  ...
  function appendAndRenderHistory(history) {
    history.forEach((message) => {
      if (message.from === "user") {
        appendMessage(message);
      } else {
